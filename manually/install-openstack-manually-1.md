...
**প্রতিটি মেশিনে করুন:**
  ...
  * **নেটওয়ার্ক কনফিগারেশন:** নেটওয়ার্ক ইন্টারফেসগুলো স্ট্যাটিক IP অ্যাড্রেস ব্যবহার করে কনফিগার করুন। `/etc/netplan/*.yaml` ফাইল এডিট করে এটি করতে পারেন।
    ...
    ```yaml
    # উদাহরণস্বরূপ: controller নোডের জন্য
    network:
      version: 2
      renderer: networkd
      ethernets:
        enp0s3: # ম্যানেজমেন্ট নেটওয়ার্ক ইন্টারফেসের নাম
          dhcp4: no
          addresses: [10.0.0.10/24]
          gateway4: 10.0.0.1 # আপনার গেটওয়ে
          nameservers:
            addresses: [8.8.8.8, 8.8.4.4]
        enp0s8: # ডেটা/টেন্যান্ট নেটওয়ার্ক ইন্টারফেসের নাম
          dhcp4: no
...
```
[mysqld]
bind-address = 10.0.0.10 # আপনার controller নোডের ম্যানেজমেন্ট IP
...
Nova ভার্চুয়াল মেশিনের জীবনচক্র পরিচালনা করে।
  ...
  * **Nova কনফিগারেশন:**
    ...
    my_ip = 10.0.0.10 # আপনার controller নোডের ম্যানেজমেন্ট IP
    use_neutron = True
...
Neutron OpenStack-এর নেটওয়ার্কিং সার্ভিস প্রদান করে।
  ...
  * **Neutron কনফিগারেশন:**
    ...
    ```
    enable_vxlan = True
    local_ip = 10.0.0.10 # আপনার controller নোডের ম্যানেজমেন্ট IP
    l2_population = True
    ```
...
Horizon হচ্ছে OpenStack-এর ওয়েব-ভিত্তিক ইউজার ইন্টারফেস।
  ...
  * **Horizon কনফিগারেশন:**
    ...
    ```python
    ALLOWED_HOSTS = ['controller', '10.0.0.10'] # আপনার কন্ট্রোলার IP
    ```
